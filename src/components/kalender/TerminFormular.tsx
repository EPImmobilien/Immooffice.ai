...
export function terminStart(t: { id: string; titel: string; art: Terminart; beginnt_am: string; endet_am: string; ganztags: boolean; ort: string | null; notiz: string | null; objekt_id: string | null; kontakt_id: string | null; zustaendig_id: string | null; teilnehmer: string[]; privat: boolean; erinnerung_minuten: number | null; nachfassen: boolean; serie_regel: SerienRegel | null; serie_id: string | null }): TerminStart {
...
export function TerminFormular({ start, mitarbeiter, objekte, kontakte, benutzerId, standardDauer = 60, weiter, kompakt = false, schliessen }: {
  ...
  const [zustand, aktion, laeuft] = useActionState<KalenderErgebnis, FormData>(terminSpeichern, {});
  const bearbeiten = Boolean(start.id);
  const [art, setArt] = useState<Terminart>(start.art ?? "besichtigung");
  ...
  return (
    <form action={aktion} className={kompakt ? "space-y-3 rounded-[var(--radius)] border border-linie bg-flaeche p-3" : "space-y-4"}>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="sm:col-span-2"><Feld id="tf-titel" beschriftung="Anlass" pflicht><Eingabe name="titel" required maxLength={300} defaultValue={start.titel ?? ""} placeholder="Besichtigung Familie Sommer" /></Feld></div>
        <Feld id="tf-art" beschriftung="Art">
          <Auswahl name="art" value={art} onChange={(e) => setArt(e.target.value as Terminart)}>{Object.entries(TERMINARTEN).map(([w, n]) => <option key={w} value={w}>{n}</option>)}</Auswahl>
        </Feld>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        {ganztags ? (
          <Feld id="tf-datum-ende" beschriftung="Bis"><Eingabe name="datum_ende" type="date" defaultValue={start.datum_ende ?? start.datum ?? datum} /></Feld>
        ) : (
          <>
            <Feld id="tf-zeit" beschriftung="Uhrzeit" pflicht><Eingabe name="zeit" type="time" required defaultValue={start.zeit ?? "10:00"} step={300} /></Feld>
            <Feld id="tf-dauer" beschriftung="Dauer">
              <Auswahl name="dauer_minuten" defaultValue={String(start.dauer ?? (art === "besichtigung" ? standardDauer : 60))}>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Feld id="tf-ort" beschriftung="Ort" hinweis="Leer = Anschrift des Objekts"><Eingabe name="ort" maxLength={300} defaultValue={start.ort ?? ""} placeholder="Vor Ort" /></Feld>
        <Feld id="tf-objekt" beschriftung="Objekt"><Auswahl name="objekt_id" defaultValue={start.objekt_id ?? ""}><option value="">— ohne —</option>{objekte.map((o) => <option key={o.id} value={o.id}>{o.bezeichnung}</option>)}</Auswahl></Feld>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Feld id="tf-zust" beschriftung="Zuständig"><Auswahl name="zustaendig_id" defaultValue={start.zustaendig_id ?? benutzerId}>{mitarbeiter.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</Auswahl></Feld>
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <Feld id="tf-erinnerung" beschriftung="Erinnerung" hinweis="Per E-Mail an Zuständigen und Teilnehmer">
          <Auswahl name="erinnerung_minuten" defaultValue={start.erinnerung_minuten === undefined ? "360" : start.erinnerung_minuten === null ? "" : String(start.erinnerung_minuten)}>{ERINNERUNGEN.map(([w, n]) => <option key={w} value={w}>{n}</option>)}</Auswahl>
        </Feld>
        <div className="flex flex-col justify-end gap-2 pb-2 sm:col-span-3 sm:flex-row sm:items-center sm:gap-5">
          {art === "besichtigung" && <label className="flex items-center gap-2 text-[13px] text-text"><input type="hidden" name="nachfassen" value="0" /><input type="checkbox" name="nachfassen" value="1" defaultChecked={start.nachfassen !== false} className="h-4 w-4 accent-akzent" />Nach der Besichtigung nachfassen (Aufgabe am Folgetag)</label>}
          <label className="flex items-center gap-2 text-[13px] text-text"><input type="checkbox" name="privat" value="1" defaultChecked={Boolean(start.privat)} className="h-4 w-4 accent-akzent" />Privat (nur Beteiligte und Verwaltung sehen Details)</label>
        </div>
      </div>

      <Feld id="tf-notiz" beschriftung="Notiz"><Textfeld name="notiz" rows={kompakt ? 2 : 3} maxLength={2000} defaultValue={start.notiz ?? ""} /></Feld>
      ...
      {bearbeiten && start.serie_id && (
        <div className="flex flex-wrap items-center gap-4 rounded-[var(--radius)] border border-linie bg-hintergrund p-3 text-[13px]">
          <span className="text-gedaempft">Serientermin ({serieText(start.serie_regel)}) — Änderung gilt für:</span>
